-- View: wavzedemo.kpi_ytd_by_user

-- DROP MATERIALIZED VIEW IF EXISTS wavzedemo.kpi_ytd_by_user;

CREATE MATERIALIZED VIEW IF NOT EXISTS wavzedemo.kpi_ytd_by_user
...
AS
 WITH leadgen AS (
         SELECT transaction_milestone_kpi.transaction_id,
           ...
           FROM wavzedemo.transaction_milestone_kpi
 ...
 SELECT t.user_id,
   ...
   FROM leadgen lg
     JOIN wavzedemo.transaction t ON t.transaction_id = lg.transaction_id
     LEFT JOIN outcome o ON o.transaction_id = lg.transaction_id
     JOIN wavzedemo.wavze_user u ON u.user_id = t.user_id
  GROUP BY t.user_id, u.preferred_name, u.last_name
WITH NO DATA;

ALTER TABLE IF EXISTS wavzedemo.kpi_ytd_by_user
